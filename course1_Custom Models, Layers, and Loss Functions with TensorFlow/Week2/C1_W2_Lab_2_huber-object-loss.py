...
def huber_loss_with_threshold(delta):
...
if __name__ == '__main__':
    ...
    # Model based on Huber loss with delta parameter
    model = Sequential([Dense(units=1, use_bias=True, input_shape=[1])])
    model.compile(optimizer='sgd', loss=huber_loss_with_threshold(0.3))
    model.fit(X, Y, epochs=500, verbose=0)
    print(model.predict([10.0]))

    # Model based on Huber loss with delta parameter
    model = Sequential([Dense(units=1, use_bias=True, input_shape=[1])])
    model.compile(optimizer='sgd', loss=HuberLoss(0.3))
    model.fit(X, Y, epochs=500, verbose=0)
    print(model.predict([10.0]))